...
class GLEPredictionMETEO:
    """
    Performing predictions of time-Series using the generalized Langevin equation (with special fit for meteo-kernel)
    This class includes
    ...
    - the reconstruction of random colored noise depending on the given memory Kernel
    ...
    def extractKernel(self, trj_array, time = None, fit_kernel = False, plot_kernel = False,fit_start = 0, fit_end = 3,kT = 2.494,p0 = (1,1,1,167,0,0)): 
        ...
        self.kernel_index = np.arange(0,int(len(self.kernel_real)*self.dt),self.dt)
    ...
    def func(self,x, a, b, c,d ,e,f):
        ...
        return a*np.exp(-b*x) + (self.kernel['k'][1]/math.e)*np.cos((2*np.pi)*x/d  - e) + f
