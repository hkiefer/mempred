...
def mp_grid_search(trj,value,cut, alphas, truncs,ts,fit_kernel = False,n_steps = 100):
    ...
    for trunc in truncs:
        for th in ts:
            for alpha in alphas:
                predict=GLEPrediction(bins=bins,  cut = cut ,trunc=trunc, dt = 1, no_fe=False, plot_pred = False)
                ...
                pred_GLE = predGLE[1][cut:cut+n_steps]
                ...
                if np.mean(error_GLE) < best_error_mean:
                    ...
                    pred_GLE_best = predGLE[1][cut:cut+n_steps]
