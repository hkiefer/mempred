...
from .memtools_funcs.flist import *
#from .ckernel import *
...
class Igle(object):
    ...
    """
    def __init__(self,xva_arg,saveall=True,prefix="",verbose=True,kT=2.494,trunc=1.,__override_time_check__=False,initial_checks=True,first_order=True,hybrid=False,corrs_from_der=False):
        """
        ...
        hybrid : bool, default=False
            Use hybrid implementation of first and second order implementation.
    ...
    def compute_kernel(self, first_order=None, hybrid=None, k0=0.):
        ...
        kernel[0] = (self.mass*a_acf[0]+au_cf[0])/v_acf[0]
        prefac = 1/v_acf[0]
        if first_order:
            #for i in range(1,len(kernel)):
                #kernel[i] = -2*prefac*((self.mass*va_cf[i] - vu_cf[i])/dt + np.sum(kernel[:i]*v_acf[1:i+1][::-1]))
            ckernel_first_order_core(v_acf,va_cf*self.mass,a_acf*self.mass,vu_cf,au_cf,dt,k0,kernel)
        #'''Commented out for mempred, because we only use first kind extraction technique'''
        elif hybrid:
            ckernel_hybrid_core(v_acf,va_cf,a_acf*self.mass,au_cf,va_cf*self.mass,vu_cf,dt,k0,kernel)
        else:
